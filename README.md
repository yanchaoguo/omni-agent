...
**最大的特点：整个框架不依赖任何第三方 Python 包。这意味着你可以把它丢进任何一台有 Python 的机器，无需 `pip install` 就能跑起来。**
...
刚入职最难受的不是活儿难，而是**没人有空带你**：业务资料散在几十个文件夹里、流程只存在老同事脑子里、想问又怕打扰人。

它不挑行业——只要你的活儿是在电脑上处理文件、数据、文档、网页，就能用。下面按行业举例：

**通用：把散落资料整理成能看懂的东西**

```
> 把这个文件夹里所有 Excel 合并成一张表，按客户名去重，标出重复项
> 这几个 PDF 合同帮我提取关键条款：签约方、金额、期限、违约条款，做成对照表
> 把这份 50 页的行业报告浓缩成 2 页要点，我要在晨会上讲
```
...
```
> 抓取这几个竞品的官网价格，整理成对比表
> 把这个月的客户拜访记录汇总，按成交概率排序，标出需要跟进的
> 帮我写一份新品推广方案，参考这份去年的方案格式
```

**财务 / 行政**

```
> 把这个月的发票信息从 PDF 里批量提取，汇总成报销明细表
> 检查这份报表里有没有前后不一致的数字
> 把这些零散的会议纪要按项目归类，生成一份月度汇总
```

**人力资源**

```
> 把这两百份简历的关键信息提取出来，按岗位匹配度排序
> 根据这份岗位说明书生成面试提纲
> 整理这份培训反馈问卷，统计各项评分并生成图表
```

**运营 / 内容**

```
> 把这份数据做成带图表的 HTML 报告，我要能直接发给老板看
> 抓取这个榜单的全部条目，整理成表格
> 帮我按这个模板批量生成 20 条商品文案
```

**教师 / 科研**

```
> 帮我把这些文献的要点整理成综述，每个结论标注来源
> 根据这份教学大纲生成课时安排表
> 把这批实验数据画成图表，按标准格式排版
```
...
```
> 我刚接手这个项目，帮我梳理整体架构和模块调用关系
> 这是我的产品设计方案，帮我找漏洞
> 看看最近 20 次提交，总结团队现在的开发重点
```
...
项目根目录的 `HAISNAP.md` 会被**自动注入上下文**。把公司规范、常用模板、业务术语写进去：

```
> 把公司的报销标准、审批流程、常用话术记到 HAISNAP.md 里
```

之后每次提问这些背景都自动生效——相当于给团队配了份"活的新人手册"，老同事也不用反复讲。

**② 把重复工作固化成"技能"**

公司里那些每周都要做一遍的活儿（周报模板、数据核对清单、客户回访流程），可以写成 `skills/` 下的 `SKILL.md`，被**语义匹配自动加载**。新人不用背流程，说一句需求它就按标准执行。
...
它会在你本机文件里搜（`grep_search` / `glob_files`），比翻网盘快得多。

**关于文件格式的支持情况**

| 格式 | 支持情况 |
|---|---|
| `.csv` `.txt` `.json` `.md` `.html` | **零依赖**，直接处理 |
| `.xlsx` Excel | 需 `openpyxl`，缺失时它会自己装 |
| `.docx` Word | 需 `python-docx`，缺失时它会自己装 |
| `.pptx` PPT | 需 `python-pptx` |
| `.pdf` | 需 `pypdf` 等库 |
| 网页内容 | `web_fetch` 直接读；动态页面用 `web_screenshot` 截图识别 |

> 遇到没装的库，它会用 `bash` 自己 `pip install`。**如果公司网络限制了 pip 源，会提示你配置镜像**，不会静默失败。

---

### 帮你改方案、挑毛病

不要只让它执行，也可以让它**质疑你的方案**：

```
> 我打算用轮询实现这个功能，你觉得有什么问题？有没有更合适的方案
> 这是我写的活动策划案，帮我找漏洞
```

它会读你本地的实际资料再回答，而不是泛泛而谈——**结论有依据**，这是它比纯聊天工具强的地方。

**适合的场景**：
- 方案写完了想找人 review，但同事都在忙
- 不确定某个做法是否有坑
- 想验证自己的想法有没有漏掉边界情况

> 提醒：它的意见是参考，不是结论。决策仍需你自己判断。
...
```
> 把这个月的文件按日期归档，超过 30 天的压缩备份
> 批量把 src 下所有文件里的旧接口名改成新的
> 给这个目录下每个文件生成一份说明文档
```

**批量改文件**这类活儿它特别合适：`multi_edit` 支持精准批量替换，改完还会**自动校验**（代码文件用 `py_compile` 复验语法），避免改崩。

**关键优势是"改完能验证"**——不是给你一段脚本让你自己跑，而是它跑通了、验证过了再交给你。
