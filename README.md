...
刚进团队最难的往往不是写代码，而是**搞不懂这个项目在干什么**。它有几条真实可用的路径：

**① 快速读懂陌生代码库**

```
> 我刚接手这个项目，帮我梳理一下整体架构，重点说明核心模块的职责和调用关系
> 主要入口在哪？改了某个文件会影响哪些地方？
```

它会读目录结构、追调用链、画模块关系，用你能听懂的话讲清楚。比硬啃几千行代码快得多。

**② 摸清项目历史与约定**

```
> 这个模块为什么要这么设计？git log 里找找线索
> 看看最近 20 次提交，总结团队现在的开发重点是什么
```

通过 `git log` / `git diff` 回溯演进脉络——很多设计的"为什么"就埋在提交信息里。

**③ 让项目自己记住约定（团队知识沉淀）**

这是它一个很实用的机制：项目根目录的 `HAISNAP.md` 会被**自动注入上下文**。

```
> 把项目的编码规范、分支策略、部署流程记到 HAISNAP.md 里
```

之后每次在这个项目里提问，这些约定都自动生效——相当于给项目配了一份"活的新人手册"。团队可以把这个文件提交进仓库，新人拉下来即继承全部上下文。

**④ 把团队规范封装成技能**

如果团队有反复要做的标准动作（比如"发版前检查清单""代码提交规范"），可以写成 `skills/` 下的 `SKILL.md`，被**语义匹配自动加载**——新人不用记规则，说一句需求它就按规范执行。
...
它会读你项目里的实际代码再回答，而不是泛泛而谈——**结论有代码依据**，这是它比纯聊天工具强的地方。
...
```
> 帮我把这个月的日志按日期归档，超过 30 天的压缩备份
> 批量把 src 下所有文件里的旧接口名改成新的
> 给这个目录下每个文件生成一份说明文档
```

**批量改代码**这类活儿它特别合适：`multi_edit` 支持精准批量替换，改完还会**自动校验语法**（用 `py_compile` 复验），避免改崩。
